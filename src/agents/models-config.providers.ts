  ...
  try {
    ...
    if (!response.ok) {
      if (process.env.OPENCLAW_DEBUG) {
        console.warn(`[ollama] Discovery failed: HTTP ${response.status} from ${apiBase}/api/tags`);
      }
      return [];
    }
    ...
    if (!data.models || data.models.length === 0) {
      if (process.env.OPENCLAW_DEBUG) {
        console.warn("[ollama] No models found on instance");
      }
      return [];
    }
    ...
    if (process.env.OPENCLAW_DEBUG) {
      console.warn(`[ollama] Discovery failed: ${String(error)}`);
    }
    return [];
  }
}
...
async function buildOllamaProvider(
  ...
  const models = await discoverOllamaModels(configuredBaseUrl, apiKey);
...
export async function resolveImplicitProviders(params: {
  ...
  if (ollamaKey) {
    ...
    const ollamaApiKeyValue =
      resolveEnvApiKey("ollama")?.apiKey ??
      resolveApiKeyFromProfiles({ provider: "ollama", store: authStore });
    providers.ollama = {
      ...(await buildOllamaProvider(ollamaBaseUrl, ollamaApiKeyValue)),
      apiKey: ollamaKey,
    };
  }
