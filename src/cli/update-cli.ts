...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
	...
	if (result.status === "skipped") {
		...
		if (result.reason === "not-git-install") {
			...
			defaultRuntime.log(
				theme.muted(
					`Examples: \`${replaceCliName("npm i -g openclaw@latest", CLI_NAME)}\` or \`${replaceCliName("bun add -g openclaw@latest", CLI_NAME)}\``,
				),
			);
		}
		defaultRuntime.exit(0);
	...
	if (shouldRestart) {
			...
			if (!opts.json && restarted) {
				...
				try {
					const interactiveDoctor =
						Boolean(process.stdin.isTTY) && !opts.json && opts.yes !== true;
		...
		defaultRuntime.log("");
		if (result.mode === "npm" || result.mode === "bun") {
			defaultRuntime.log(
				theme.muted(
