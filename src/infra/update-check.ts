...
function resolveDepsMarker(params: { root: string; manager: PackageManager }): {
	...
	if (params.manager === "bun") {
		return {
			lockfilePath: path.join(root, "bun.lock"),
...
export async function checkDepsStatus(params: {
	...
	const root = path.resolve(params.root);
	const { lockfilePath: baseLockfilePath, markerPath } = resolveDepsMarker({
		root,
		manager: params.manager,
	});

	let lockfilePath = baseLockfilePath;
	...
	let lockExists = await exists(lockfilePath);
	if (!lockExists && params.manager === "bun") {
		const legacyLockfilePath = path.join(root, "bun.lockb");
		if (await exists(legacyLockfilePath)) {
			lockfilePath = legacyLockfilePath;
			lockExists = true;
		}
	}
