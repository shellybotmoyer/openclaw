...
describe("runGatewayUpdate", () => {
	...
	it("uses stable tag when beta tag is older than release", async () => {
		...
		await fs.writeFile(
			path.join(tempDir, "package.json"),
			JSON.stringify({ name: "openclaw", version: "1.0.0", packageManager: "bun@8.0.0" }),
			"utf-8",
		);
		...
		const { runner, calls } = createRunner({
			...
			"bun install": { stdout: "" },
			"bun run build": { stdout: "" },
			"bun run ui:build": { stdout: "" },
	...
	it("skips update when no git root", async () => {
		await fs.writeFile(
			path.join(tempDir, "package.json"),
			JSON.stringify({ name: "openclaw", packageManager: "bun@8.0.0" }),
			"utf-8",
		);
		...
		expect(calls.some((call) => call.startsWith("bun add -g"))).toBe(false);
	...
	it("repairs UI assets when doctor run removes control-ui files", async () => {
		...
		const runCommand = async (argv: string[]) => {
			...
			if (key === "bun run build") {
				return { stdout: "", stderr: "", code: 0 };
			}
			if (key === "bun run ui:build") {
				uiBuildCount += 1;
				await fs.mkdir(path.dirname(uiIndexPath), { recursive: true });
