...
describe("control UI assets helpers", () => {
	...
	it("resolves repo root when argv1 is a symlink", async () => {
		const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-ui-"));
		try {
			const realPkg = path.join(tmp, "real-pkg");
			const bin = path.join(tmp, "bin");
			await fs.mkdir(path.join(realPkg, "ui"), { recursive: true });
			await fs.writeFile(path.join(realPkg, "ui", "vite.config.ts"), "export {};\n");
			await fs.writeFile(
				path.join(realPkg, "package.json"),
				JSON.stringify({ name: "openclaw" }),
			);
			...
			expect(
				await canonicalPath(resolveControlUiRepoRoot(path.join(bin, "openclaw")) ?? ""),
			).toBe(await canonicalPath(realPkg));
		} finally {
			await fs.rm(tmp, { recursive: true, force: true });
		}
	});
