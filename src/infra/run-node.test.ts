	...
	it.runIf(process.platform !== "win32")(
		...
		async () => {
			await withTempDir(async (tmp) => {
				...
				const argsPath = path.join(tmp, ".bun-args.txt");
...
fs.writeFileSync(path.join(cwd, ".bun-args.txt"), args.join(" "), "utf-8");
...
`;
				await fs.writeFile(fakeBunPath, fakeBun, "utf-8");
				await fs.chmod(fakeBunPath, 0o755);
				...
				await expect(fs.readFile(argsPath, "utf-8")).resolves.toContain("x tsdown --no-clean");
