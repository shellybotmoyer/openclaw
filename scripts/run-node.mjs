...
// Bun Direct Mode: Skip compilation entirely
const useDirectMode = env.OPENCLAW_BUN_DIRECT === "1";
...
const shouldBuild = () => {
  // In direct mode, skip build check entirely
  if (useDirectMode) {
    return false;
  }
...
const runNode = () => {
  // In direct mode, use bun directly instead of node
  const execCmd = useDirectMode ? "bun" : process.execPath;
  const nodeProcess = spawn(execCmd, ["openclaw.mjs", ...args], {
    cwd,
    env,
    stdio: "inherit",
  });
...
// Direct mode: skip build and run immediately
if (useDirectMode) {
  logRunner("Bun Direct Mode: Skipping tsdown compilation.");
  runNode();
} else if (!shouldBuild()) {
  runNode();
} else {
  logRunner("Building TypeScript (dist is stale).");
  const buildCmd = process.platform === "win32" ? "bun.exe" : "bun";
  const build = spawn(buildCmd, compilerArgs, {
    cwd,
    env,
    stdio: "inherit",
  });
