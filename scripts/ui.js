...
function usage() {
	// keep this tiny; it's invoked from package scripts.
	process.stderr.write("Usage: bun scripts/ui.js <install|dev|build|test> [...args]\n");
}
...
if (action === "install") {
	const args = runner.kind === "bun" ? ["install", ...rest] : ["install", ...rest];
	run(runner.cmd, args);
} else {
